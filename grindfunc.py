...
def calculate_values(samples, variables, values):
    ...
    for v in variables:
        for i in range(v["n_values"]):
            values[v["offset_values"]+i]=twodec((statistics.mean(samples[v["offset_samples"]+v["n_samples"]*i:v["offset_samples"]+v["n_samples"]*(i+1)])-samples_mean)/samples_mean)
    #print(values)


def get_target(samples, target_definition):
    target_samples=[row[1] for row in samples]
    target_samples=target_samples[-target_definition["samples_to"]:-target_definition["samples_from"]]
    target_samples.sort(reverse=True)
    #print(samples[-1])
    return quarterdec(100*(statistics.mean(target_samples[:target_definition["n_samples"]])-samples[-1][1])/samples[-1][1])

def import_samples(datefrom="2021-01-01", dateto="9999"):
    df=pd.read_csv("Binance_BTCUSDT_minute13.csv",usecols=[1, 3],parse_dates=[1],skiprows=1)
    #print(df.head())
